 ...
 */
public class IfcRelDeclares<T> extends IfcRelationship {
    public final static String TAG = IfcRelDeclares.class.getSimpleName();
    private static Logger logger = Logger.getLogger(TAG);
    private IfcObjectDefinition relatingObject;
    private List<T> relatedObjects;


    public IfcRelDeclares(Class clazz) {
        if (isTypeSupported(clazz)) {
            relatedObjects = new ArrayList<T>();
        }
    }

    public IfcRelDeclares(IfcObjectDefinition relatingObject, Class clazz) {
        this.relatingObject = relatingObject;
        relatedObjects = new ArrayList<T>();
    }

    public IfcRelDeclares(IfcObjectDefinition relatingObject, List<T> relatedObjects, Class clazz) {
        this.relatingObject = relatingObject;
